...
const formatDate = (dateString) => {
  // 7. Añadida comprobación para fechas nulas
  if (!dateString) return 'N/A';
  const options = { year: 'numeric', month: '2-digit', day: '2-digit' };
  return new Date(dateString).toLocaleDateString('es-ES', options);
};

const Get_HistorialReproduccion = () => {
      ...
      <div className="tabla-scroll-container">
        {loading ? (
          ...
          <>
                ...
                {currentRecords.length > 0 ? (
                  currentRecords.map((item) => (
                    <tr key={item._id}>
                      ...
                      <td>
                        {/* 9. 'edit-button' ya coincide con tu CSS */}
                        <Link 
                          to={`/historial/editar/${item._id}`} 
            ...
            {/* 10. La paginación no tiene estilos en tu CSS */}
...
export default Get_HistorialReproduccion;